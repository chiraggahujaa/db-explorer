...
export class SupabaseConnection extends BaseDatabaseConnection {
  ...
  async connect(): Promise<void> {
    try {
      const url = this.config.url || this.config.projectUrl;
      const baseUrl = url?.replace(/\/+$/, '');
      const apiKey = this.config.anonKey;
      const serviceKey = this.config.serviceRoleKey || this.config.serviceKey;
  ...
  async query(sql: string, params: any[] = []): Promise<QueryResult> {
    ...
    try {
      // For Supabase, we'll use RPC for complex queries
      // For simple SELECT queries, we can use PostgREST
      ...
      // Simple SELECT queries via PostgREST
  ...
  override async listDatabases(): Promise<string[]> {
    try {
      // For Supabase, we'll return the common schemas
      return ['public'];
    } catch (error) {
  ...
  private parseSimpleQuery(sql: string): { type: string; table?: string; columns?: string; where?: string; limit?: number; joins?: string } {
    ...
    if (selectMatch) {
      ...
      const result: { type: string; table?: string; columns?: string; where?: string; limit?: number; joins?: string } = {
        type: 'SELECT',
        table,
        columns,
        where,
